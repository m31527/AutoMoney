...
$('ai-info').textContent=ai.model+' · 呼叫 '+ai.calls+' 次 · 失敗 '+ai.failures+' 次 · 成交 '+ai.trades+' 次'+(ai.prefilter_enabled?' · 空倉預篩已啟用':'')+(ai.prefilter_skipped?' · 累計省略 '+ai.prefilter_skipped+' 次推論':'')+(ai.policy_version==='ai-budget-freshness-v2'?' · 預算／行情檢查 v2':'')+(ai.last_inference_seconds!=null?' · 最近推論 '+num(ai.last_inference_seconds,1)+' 秒':'');
